...
def process_dct_img(img):
    ...
    new_img = F.interpolate(fft_img, size=[250, 1])  # [batch_size, 64, 250, 1]    #会丢失一些特征，或许可以将250调大，或者尝试别的特征提取方法
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key = key.view(-1, self.model_dim, num_heads, dim_per_head).transpose(1, 2)
        value = value.view(-1, self.model_dim, num_heads, dim_per_head).transpose(1, 2)
        query = query.view(-1, self.model_dim, num_heads, dim_per_head).transpose(1, 2)
        ...
        attention = attention.transpose(1, 2).contiguous().view(-1, self.model_dim, dim_per_head * num_heads)
...
class NetShareFusion(nn.Module):
    def __init__(self,
        ...
        # classifier
        self.linear1 = nn.Linear(model_dim, 1024)
...
class TrainALL(tune.Trainable):
    def get_dataloader(self):
        ...
        elif self.dataset_name == 'extend':
            import data_process_extend as pro
        else:
        ...
        elif self.dataset_name == 'weibo' or self.dataset_name == 'extend':
            transform_vgg = transforms.Compose([
                transforms.Resize((224, 224)),
                transforms.ToTensor(),
            ])
    ...
    def setup(self, config):
        ...
        self.CASED = '/home/tanhz/Reproduction_of_MCAN/models/bert-base-chinese'  # multilingual-cased
        self.VOCAB = '/home/tanhz/Reproduction_of_MCAN/models/bert-base-chinese/vocab.txt'
        # self.CASED = '/home/xxxx/models/bert-base-multilingual-cased/'  # multilingual-cased
        # self.VOCAB = '/home/tanhz/Reproduction_of_MCAN/models/bert-base-multilingual-cased/vocab.txt'
        self.pthfile = '/home/tanhz/Reproduction_of_MCAN/models/vgg19-dcbb9e9d.pth'
        self.save_root = '/home/tanhz/Reproduction_of_MCAN/results'
        ...
        self.num_layers = config.get("num_layers")  # int, e.g, 1d
    ...
    def print_result_table_handler(self, loss_values, acc_values,
        ...
        # 这个地方需要绝对路径，不能使用相对路径
        if not os.path.exists('/home/tanhz/Reproduction_of_MCAN/results'):
            os.makedirs('/home/tanhz/Reproduction_of_MCAN/results')
        f_result = open(
            '/home/tanhz/Reproduction_of_MCAN/results/{}_accuracy_without_bert.txt'.format(
                self.dataset_name), 'a+')
    ...
    def condition_save(self, epoch_index, test_epoch_acc, report):
        ...
        if self.dataset_name == 'twitter':
            ...
            if test_epoch_acc >= weibo_threshold:
                folder_path = 'model_save_weibo_without_bert'
                self.save_model(folder_path, epoch_index, test_epoch_acc, report)
        elif self.dataset_name == 'extend':
            if test_epoch_acc >= extend_threshold:
                folder_path = 'model_save_extend_without_picture'
                self.save_model(folder_path, epoch_index, test_epoch_acc, report)
