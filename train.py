...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #     gray_array = np.full_like(np.array(img), 127)  # 将图像变为全灰，像素值127
...
class TrainALL(tune.Trainable):
    ...
    def setup(self, config):
        ...
        self.CASED = '~/Reproduction_of_MCAN/models/bert-base-chinese'
        self.VOCAB = '~/Reproduction_of_MCAN/models/bert-base-chinese/vocab.txt'
        # self.CASED = '~/Reproduction_of_MCAN/models/bert-base-multilingual-cased/'  # multilingual-cased
        # self.VOCAB = '~/Reproduction_of_MCAN/models/bert-base-multilingual-cased/vocab.txt'
        self.pthfile = '~/Reproduction_of_MCAN/models/vgg19-dcbb9e9d.pth'
        self.save_root = '~/Reproduction_of_MCAN/results'
    ...
    def print_result_table_handler(self, loss_values, acc_values,
        ...
        # 这个地方需要绝对路径，不能使用相对路径
        if not os.path.exists('~/Reproduction_of_MCAN/results'):
            os.makedirs('~/Reproduction_of_MCAN/results')
        f_result = open(
            '~/Reproduction_of_MCAN/results/{}_accuracy_test.txt'.format(
                self.dataset_name), 'a+')
